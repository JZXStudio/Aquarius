...
//  Aquarius
//
//  Created by JZXStudio on 2025/6/3.
//
...
open class ALocation: NSObject {
    ...
    public func reverseGeocodeLocation(_ loc: CLLocation, _ placemark: ((_ placemark: [CLLocation]) -> Void)?=nil, fail: ((_ error: Error) -> Void)? = nil) {
        ...
        // loc: CLLocation对象
        let point = ALocationTransform.wgs2gcj(wgsLat: loc.coordinate.latitude, wgsLng: loc.coordinate.longitude)
        let newLocation = CLLocation(latitude: point.gcjLat, longitude: point.gcjLng)
...
public struct ALocationTransform {
